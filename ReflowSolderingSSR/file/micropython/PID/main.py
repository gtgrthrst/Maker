...
class SimulatedSystem:
    ...
    def update(self, control):
        self.value += control
        return self.value
controlled_system = SimulatedSystem()
pid = PID(10, 0.05, 200, setpoint=1, scale='us')
...
#     print("T1: ", T1)
    ...
    T3 = enc.value() + 150
    pid.setpoint = T3
#     print("pid.setpoint" ,T3)
    pid.output_limits = (0, 5)    # Output value will be between 0 and 10
    control = pid(T1)
    print(T1,",",T3,",",control*10,",","\n")
    
    v = controlled_system.update(control)
#     print(pid._last_time,",",pid.setpoint,",")
    if T3 != T4:
        display.fill_rect(80,30, 128, 10, 0)  #刪除填充
        T4 = T3
        display.text( str(T3), 80,30 , 1)
